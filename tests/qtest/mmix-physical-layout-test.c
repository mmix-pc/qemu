...
static void test_mmix_hosted_arguments_reset(void)
{
    ...
    enum {
        ...
        STRING_OFFSET = sizeof(uint64_t) + POINTER_BYTES,
        FIRST_BYTES = QEMU_ALIGN_UP(sizeof(first), sizeof(uint64_t)),
        SECOND_BYTES = QEMU_ALIGN_UP(sizeof(second), sizeof(uint64_t)),
        ARGUMENT_BYTES = STRING_OFFSET + FIRST_BYTES + SECOND_BYTES,
    };
    ...
    base = mmix_hmp_register_value(registers, "r1  ") - sizeof(uint64_t);
    g_assert_cmphex(base % (8 * KiB), ==, 0);

    stq_be_p(expected, base + ARGUMENT_BYTES);
    stq_be_p(expected + sizeof(uint64_t),
             base + STRING_OFFSET);
    stq_be_p(expected + 2 * sizeof(uint64_t),
             base + STRING_OFFSET + FIRST_BYTES);
    memcpy(expected + STRING_OFFSET, first, sizeof(first));
    memcpy(expected + STRING_OFFSET + FIRST_BYTES, second, sizeof(second));
    ...
    g_assert_cmphex(mmix_hmp_register_value(registers, "r0  "), ==,
                    ARGUMENT_COUNT);
    g_assert_cmphex(mmix_hmp_register_value(registers, "r1  "), ==,
