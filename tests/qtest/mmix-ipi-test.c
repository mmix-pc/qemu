...
#define MMIX_VIRT_IPI_BASE 0x10006000ULL
#define MMIX_VIRT_IPI_ACTIVE_TARGETS 0x0000
#define MMIX_VIRT_IPI_SEND 0x0008
#define MMIX_VIRT_IPI_CONTEXT_BASE 0x0100
#define MMIX_VIRT_IPI_CONTEXT_STRIDE 0x20
...
{
    QTestState *qts = mmix_ipi_start(16);

    g_assert_cmphex(mmix_ipi_read_active_targets(qts), ==, 0xffff);
    mmix_ipi_send(qts, 1U << 15);
    g_assert_cmphex(mmix_ipi_read_status(qts, 15), ==,
                    MMIX_VIRT_IPI_STATUS_PENDING);
    g_assert_false(qtest_get_irq(qts, 14));
    g_assert_true(qtest_get_irq(qts, 15));

    mmix_ipi_clear(qts, 15, MMIX_VIRT_IPI_STATUS_PENDING);
    g_assert_false(qtest_get_irq(qts, 15));
